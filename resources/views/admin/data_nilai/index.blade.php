                ...
                @foreach($data_nilais as $data_nilai)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="flex items-center justify-center px-4 py-2 space-x-2">
                            <a href="{{ route('nilai.edit', $data_nilai->anggota_id) }}" class="flex items-center px-2 py-1 text-white bg-yellow-400 rounded hover:bg-yellow-500">Edit</a>
                            <form action="{{ route('nilai.destroy', $data_nilai->anggota_id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data nilai anggota ini?');">
                            @csrf
                            @method('DELETE')
